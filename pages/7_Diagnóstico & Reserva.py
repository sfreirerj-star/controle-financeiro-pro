...
if not df_lancamentos.empty and "data" in df_lancamentos.columns:
    ...
    df_lancamentos["data_dt"] = pd.to_datetime(df_lancamentos["data"], errors="coerce", dayfirst=True)
    mask = df_lancamentos["data_dt"].isna()
    if mask.any():
        df_lancamentos.loc[mask, "data_dt"] = pd.to_datetime(df_lancamentos.loc[mask, "data"], errors="coerce")
...
if competencia_selecionada != "Todos os Meses" and not df_lancamentos.empty and "competencia" in df_lancamentos.columns:
    df_filtrado = df_lancamentos[df_lancamentos["competencia"] == competencia_selecionada]
else:
    df_filtrado = df_lancamentos.copy()
# ==========================================================

if df_filtrado.empty:
    st.info(f"Nenhum lançamento encontrado para {competencia_selecionada}. Selecione 'Todos os Meses' no menu lateral.")
else:
