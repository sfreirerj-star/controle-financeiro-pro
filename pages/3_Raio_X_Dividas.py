...
st.set_page_config(
    page_title="Raio-X de Dívidas — Painel do Marcelo", page_icon="⚠️", layout="wide"
)
...
st.subheader("⚠️ Gerenciamento de Dívidas e Parcelamentos (Raio-X)")

try:
  conexao_sidebar = obter_conexao()
  competencia_selecionada, _ = configurar_sidebar_competencia(
      conexao_sidebar, prefixo_key="raio_x_dividas"
  )
...
if not df_dividas.empty:
  ...
  with col_m2:
    st.metric(
        label="⏳ Prazo Estimado para Quitação Total", value=prazo_texto
    )
  ...
  with col_e1:
    with st.form("form_edicao_divida"):
      ...
      novo_credor = st.text_input(
          "Credor",
          value=str(dado_atual["credor"])
          if pd.notna(dado_atual["credor"])
          else "",
      )
      novo_valor_total = st.number_input(
          "Valor Total (R$)",
          value=float(dado_atual["valor_total"])
          if "valor_total" in dado_atual and pd.notna(dado_atual["valor_total"])
          else 0.0,
          format="%.2f",
      )
      novo_total_parcelas = st.number_input(
          "Total de Parcelas",
          value=int(dado_atual["total_parcelas"])
      ...
      novo_valor_parcela = st.number_input(
          "Valor da Parcela (R$)",
          value=float(dado_atual["valor_parcela"])
      ...
      novo_dia_vencimento = st.number_input(
          "Dia de Vencimento",
          value=int(dado_atual["dia_vencimento"])
